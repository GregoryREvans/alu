...
melodic_sequence = [
    int(_)
    for _ in evans.Sequence([11, 10, 4, 6, 5, 7, 1, 3, 2, 0, 9, 8])
    .matrix()
    .potamia(columns=False, retrograde=False)
    .flatten()
]

source_chord = ["b", "ds'", "fs'", "bf'", "ef''", "a''", "c'''", "g'''"]

reordering_row = evans.PitchClassSegment(
    [
...
reordering_series = evans.Sequence(
    [abjad.NumberedPitch(int(_)) for _ in reordering_row]
)
...
melodic_sequence = [
    ...
    for _ in evans.Sequence([11, 10, 4, 6, 5, 7, 1, 3, 2, 0, 9, 8])
    .matrix()
    .potamia(columns=True, retrograde=False)
    .flatten()
]

reordering_series = evans.Sequence(
    [abjad.NumberedPitch(int(_)) for _ in reordering_row.invert(8)]
)
...
melodic_sequence = [
    ...
    for _ in evans.Sequence([11, 10, 4, 6, 5, 7, 1, 3, 2, 0, 9, 8])
    .matrix()
    .potamia(columns=False, retrograde=True)
    .flatten()
]

reordering_series = evans.Sequence(
    [abjad.NumberedPitch(int(_)) for _ in reordering_row.retrograde()]
)
...
melodic_sequence = [
    ...
    for _ in evans.Sequence([11, 10, 4, 6, 5, 7, 1, 3, 2, 0, 9, 8])
    .matrix()
    .potamia(columns=True, retrograde=True)
    .flatten()
]

reordering_series = evans.Sequence(
    [abjad.NumberedPitch(int(_)) for _ in reordering_row.invert(8).retrograde()]
)
...
reordering_series = evans.Sequence(
    [abjad.NumberedPitch(int(_)) for _ in reordering_row.retrograde().alpha(1)]
)
...
handler_1 = evans.PitchHandler(
    evans.Sequence(abjad.sequence.rotate(pitches_1, 0)).transpose(-12)
)
handler_1(music_1)
music_1 = abjad.mutate.eject_contents(music_1)

handler_2 = evans.PitchHandler(
    evans.Sequence(abjad.sequence.rotate(pitches_2, -2)).transpose(-36)
)
handler_2(music_2)
music_2 = abjad.mutate.eject_contents(music_2)

handler_3 = evans.PitchHandler(
    evans.Sequence(abjad.sequence.rotate(pitches_3, -4)).transpose(-24)
)
handler_3(music_3)
music_3 = abjad.mutate.eject_contents(music_3)

handler_4 = evans.PitchHandler(
    evans.Sequence(abjad.sequence.rotate(pitches_4, -6)).transpose(-24)
)
handler_4(music_4)
music_4 = abjad.mutate.eject_contents(music_4)

handler_5 = evans.PitchHandler(
    evans.Sequence(abjad.sequence.rotate(pitches_5, -8)).transpose(-24)
)
handler_5(music_5)
